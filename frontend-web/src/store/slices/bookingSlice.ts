  ...
  reducers: {
    ...
    setAllBookings: (state, action: PayloadAction<Reservation[]>) => {
      ...
      state.pastBookings = action.payload.filter(booking => {
        const endTime = new Date(booking.endTime);
        return endTime <= now || booking.status === 'cancelled' || booking.status === 'completed';
      });
    ...
    updateBooking: (state, action: PayloadAction<{ id: string; updates: Partial<Reservation> }>) => {
      ...
      const allIndex = state.allBookings.findIndex(booking => booking.id === id);
      if (allIndex !== -1) {
        state.allBookings[allIndex] = { ...state.allBookings[allIndex], ...updates };
      }

      // Update in upcoming bookings
      const upcomingIndex = state.upcomingBookings.findIndex(booking => booking.id === id);
      if (upcomingIndex !== -1) {
    ...
    deleteBooking: (state, action: PayloadAction<string>) => {
      ...
      state.allBookings = state.allBookings.filter(booking => booking.id !== id);
      state.upcomingBookings = state.upcomingBookings.filter(booking => booking.id !== id);
      state.pastBookings = state.pastBookings.filter(booking => booking.id !== id);
    },
