...
class OrganizationController extends BaseController {
  ...
  getOrganizationById = async (req, res, next) => {
    ...
    console.log(
      `Organization request - orgId: ${orgId}, requestingUser.organizationId: ${requestingUser.organizationId}, requestingUser.role: ${requestingUser.role}`
    );
  ...
  updateOrganization = async (req, res, next) => {
    ...
    const validationResult =
      this.organizationValidator.validateUpdateOrganization(req.body);
    if (!validationResult.success) {
      return this.failureResponse("Validation failed", next, 422);
    }

    const organization = await this.organizationRepo.updateOrganization(
      orgId,
  ...
  getOrganizationStats = async (req, res, next) => {
    ...
    if (
      ...
      return this.failureResponse(
        "Only organization admins can view organization stats",
        next,
        403
      );
    }
  ...
  regenerateInviteKey = async (req, res, next) => {
    ...
    if (
      ...
      return this.failureResponse(
        "Only organization admins can regenerate invite key",
        next,
        403
      );
    }
  ...
  getInviteKey = async (req, res, next) => {
    ...
    if (
      ...
      return this.failureResponse(
        "Only organization admins can view invite key",
        next,
        403
      );
    }
