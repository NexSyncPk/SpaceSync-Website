...
class UserController extends BaseController {
    ...
    register = async (req, res, next) => {
        ...
        if (existingUser) {
            return this.failureResponse('User already exists with this email', next, 409);
        }
        ...
        return this.successResponse(res, 'User registered successfully', {
            user,
    ...
    login = async (req, res, next) => {
        ...
        const isPasswordValid = await this.userRepo.comparePassword(password, user.password);
        if (!isPasswordValid) {
            return this.failureResponse('Invalid email or password', next, 401);
        }
    ...
    createOrganization = async (req, res, next) => {
        ...
        if (user.organizationId) {
            return this.failureResponse('You already belong to an organization', next, 409);
        }

        if (user.role !== 'unassigned') {
            return this.failureResponse('Only unassigned users can create organizations', next, 403);
        }

        const { name } = req.body;
        if (!name || name.trim().length < 2) {
            return this.failureResponse('Organization name is required and must be at least 2 characters', next, 422);
        }

        const organization = await this.organizationRepo.createOrganization({ name: name.trim() });
        ...
        return this.successResponse(res, 'Organization created successfully. You are now an admin!', {
            organization,
            user: updatedUser,
    ...
    joinOrganization = async (req, res, next) => {
        ...
        if (user.role !== 'unassigned') {
            return this.failureResponse('Only unassigned users can join organizations', next, 403);
        }
