...
class ReservationValidator extends BaseValidator {
  ...
  createReservationSchema = Joi.object({
    ...
    agenda: Joi.string().min(5).max(500).required().messages({
    ...
    requiredAmenities: Joi.array()
      .items(
        Joi.string()
          ...
          .messages({
            "any.only":
              "Invalid amenity. Must be one of: displayProjector, displayWhiteboard, cateringAvailable, videoConferenceAvailable",
          })
      )
      .default([])
      .messages({
  ...
  checkAvailabilitySchema = Joi.object({
    ...
    excludeReservationId: Joi.string().uuid().messages({
  ...
  validateCreateReservation = (data) =>
    this.validate(this.createReservationSchema, data);
  validateUpdateReservation = (data) =>
    this.validate(this.updateReservationSchema, data);
  validateSearchReservation = (data) =>
    this.validate(this.searchReservationSchema, data);
  validateCheckAvailability = (data) =>
    this.validate(this.checkAvailabilitySchema, data);
